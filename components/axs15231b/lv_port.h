 ...
 *
 * @note This function initializes LVGL and creates a timer and task for LVGL to work correctly.
 *
 * @return
 *      - ESP_OK                    on success
 ...
 * Some deinitialization will be done after the task is stopped.
 *
 * @return
 *      - ESP_OK                    on success
 */
 ...
 *
 * @note Allocated memory in this function is not freed in deinit. You must call lvgl_port_remove_disp to free all memory!
 *
 ...
 *
 * @note Allocated memory in this function is not freed in deinit. You must call lvgl_port_remove_touch to free all memory!
 *
 * @param touch_cfg Touch configuration structure
 * @return Pointer to LVGL touch input device or NULL when an error occurred
 */
...
/**
 * @brief Read touch data for Waveshare 1.69" LCD Touch ESP32-S3R8
 *
